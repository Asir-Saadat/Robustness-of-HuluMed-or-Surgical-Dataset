#!/usr/bin/env python3
"""
Grounded questions accuracy evaluation script for Gemini answers.
"""
...
# Load gemini grounded answers
json_file = '/home/as5606/projects/Hulu-Med/gemini/gemini_grounded_answers.json'
...
for item in tqdm(gemini_data, desc="Comparing grounded answers"):
    item_id = item['id']
    image = item['image']
    ...
    for grounded_qa in item['grounded_qa_with_gemini_answers']:  # ✅ Fixed key
        question = grounded_qa['question']
        expected_answer = grounded_qa['original_answer']
        gemini_answer = grounded_qa['gemini_answer']  # ✅ Fixed field name
        
        match_result = compare_answers(expected_answer, gemini_answer)
...
# Save results
output_path = '/home/as5606/projects/Hulu-Med/gemini/gemini_grounded_accuracy.json'
...
# Calculate accuracy
try:
    total_qa = sum(len(item['grounded_qa_with_matches']) for item in results)
    match_count = sum(sum(1 for qa in item['grounded_qa_with_matches'] if qa['match'] == "MATCH") for item in results)
    accuracy = (match_count / total_qa) * 100
