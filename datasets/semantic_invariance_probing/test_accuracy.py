#!/usr/bin/env python3
"""
Semantic variations accuracy evaluation script.
Evaluates if Hulu-Med answers are consistent across semantic variations.
"""
...
# Load the appropriate semantic variations answers file based on model size
json_file = f'/home/as5606/Datasets/Cholec_text_semantic_similar_questions/hulu_med_{model_size}_semantic_variations_answers.json'
...
for item in tqdm(hulumed_data, desc="Comparing semantic variation answers"):
    item_id = item['id']
    image = item['image']
    original_question = item['original_question']
    original_answer = item['original_answer']

    # Compare each semantic variation answer with original answer
    semantic_variations_with_matches = []
    for semantic_qa in item['semantic_variations_with_hulu_answers']:
        question = semantic_qa['question']
        hulu_answer = semantic_qa['hulu_med_answer']

        # Compare original answer with semantic variation answer
        match_result = compare_answers(original_answer, hulu_answer)

        semantic_variations_with_matches.append({
            'question': question,
    ...
    result = {
        ...
        'semantic_variations_with_matches': semantic_variations_with_matches
    }
    results.append(result)

# Save to JSON FIRST (before any counting that might fail)
output_path = f'/home/as5606/Datasets/Cholec_text_semantic_similar_questions/hulu_med_{model_size}_semantic_variations_accuracy.json'
...
try:
    total_semantic_qa = sum(len(item['semantic_variations_with_matches']) for item in results)
    match_count = sum(sum(1 for qa in item['semantic_variations_with_matches'] if qa['match'] == "MATCH") for item in results)
    accuracy = (match_count / total_semantic_qa) * 100

    print(f"\n{'='*50}")
    print(f"Model: Hulu-Med-{model_size}")
    print(f"Total semantic variation QA pairs: {total_semantic_qa}")
    print(f"Matched with original answer: {match_count}")
